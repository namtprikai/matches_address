...
rem Building IF003
poetry run pyinstaller --onefile --noconsole --distpath ./dist --collect-all chardet --collect-all pandas --collect-all geopandas --collect-all shapely --collect-all lightgbm --collect-all numpy --collect-all sklearn --collect-all fiona --collect-all pyogrio --add-data "async_tasks:async_tasks" --add-data "src:src" --paths="./src" --name IF003 ./async_tasks/IF003.py
