...
try {
  ...
  const commands = [
    ...
    `poetry run pyinstaller --onefile --distpath ${DIST_DIR} --collect-all chardet --collect-all pandas --collect-all geopandas --collect-all shapely --collect-all lightgbm --collect-all numpy --collect-all sklearn --add-data="async_tasks${separator}async_tasks" --add-data="src${separator}src" --paths="${SRC_DIR}" --name IF003 ./async_tasks/IF003.py`,
