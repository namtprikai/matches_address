...
try {
  ...
  // PyInstallerコマンドの実行
  const commands = [
    `poetry run pyinstaller --onefile --distpath ${DIST_DIR} --collect-all numpy --collect-all fiona --collect-all chardet --collect-all pandas --collect-all geopandas --collect-all pyogrio --collect-all shapely --collect-all japanese_address --collect-all rapidfuzz --add-data="src${separator}src" --paths="${SRC_DIR}" --name IF001 ./async_tasks/IF001.py`,
