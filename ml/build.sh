...
poetry run pyinstaller --onefile --distpath $DIST_DIR --collect-all numpy --collect-all chardet --collect-all pandas --collect-all geopandas --collect-all pyogrio --collect-all shapely --add-data "src:src" --paths="$SRC_DIR" --name IF001 ./async_tasks/IF001.py

poetry run pyinstaller --onefile --distpath $DIST_DIR --collect-all imblearn --collect-all japanize_matplotlib --collect-all memory_profiler --collect-all chardet --collect-all pandas --collect-all sklearn --collect-all lightgbm --collect-all numpy --collect-all optuna --collect-all seaborn --collect-all japanize_matplotlib --add-data "src:src" --paths="$SRC_DIR" --name IF002 ./async_tasks/IF002.py
