...
if __name__ == "__main__":
    ...
    with gr.Blocks() as iface:
        akiya_pred_file = gr.File(label="【D902】空き家判定結果データ", file_types=[".zip", ".gpkg", ".geojson", ".csv"])
        spatial_file = gr.File(label="【D013】国勢調査小地域データ（町丁・字等）", file_types=[".zip", ".gpkg", ".geojson", ".csv"])
        key_column = gr.Dropdown(label="集計に使用するカラム", choices=[])
