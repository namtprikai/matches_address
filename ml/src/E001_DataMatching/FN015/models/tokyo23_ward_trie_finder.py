...
from FN015.services.utils_db import get_tokyo23_wards_generator_hash, get_tokyo23_ward


class Tokyo23WardTrieFinder(TrieAddressFinder2):
    @staticmethod
    ...
    def get_cache_file_path(di_container):
        ...
        gen_hash = get_tokyo23_wards_generator_hash()
        return os.path.join(di_container['cache_dir'], f"tokyo23-ward_{gen_hash}.abrg2")

    @staticmethod
    def create_dictionary_file(di_container):
        cache_file_path = Tokyo23WardTrieFinder.get_cache_file_path(di_container)
        # Xóa cache cũ
        for f in os.listdir(di_container['cache_dir']):
            if f.startswith("tokyo23-ward_") and f.endswith(".abrg2"):
                os.remove(os.path.join(di_container['cache_dir'], f))

        rows = get_tokyo23_ward()
            ...
            try:
                if os.path.exists(cache_file_path):
                    with open(cache_file_path, 'rb') as f:
                        data = f.read()
                    Tokyo23WardTrieFinder(bytearray(data[:100]))
                    return data
            except Exception as e:
                print(e)
            if not Tokyo23WardTrieFinder.create_dictionary_file(di_container):
                return None
            num_of_try += 1
        return data 