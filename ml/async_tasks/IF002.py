...
def main():
    ...
    try:
        ...
        job_id = create_or_update_job(job_id ,"", "ml", os.getpid(), 0, args.parameters, True)

        params = {
            'db_path': database_path,
            'input_path': json_dict.get('input_path', None),
            'output_path': json_dict.get('output_path', '.'),
            'isolation_forest_min_count': json_dict.get('outlier_threshold_min', 0.01),
            'isolation_forest_max_count': json_dict.get('outlier_threshold_max', 0.10),
            'isolation_forest_step': json_dict.get('outlier_threshold_step', 0.01),
