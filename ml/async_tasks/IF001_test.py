...
def main():
    ...
    json_dict = {
        ...
        "data": {
            ...
            "reverse_geocoded_building_polygon": {
                "id": 2,
                "path": "reverse_geocoded_building_polygon.csv",
                "columns": {
                    "address": "new_address",
                    "geometry": "geometry",
                }
            ...
            "census": {
                "id": 7,
                "path": "国勢調査_小地域_シェープファイル_2020.zip"
            },
        ...
        "output_path": "D:\\source_code\\links-akiya\\app\\database",
        "database_path": "D:\\source_code\\links-akiya\\app\\database\\database.db"
    }
    job_id = None
    ...
    try:
        ...
        job_id = create_or_update_job(job_id,"", "preprocess", os.getpid(), 0, json.dumps({}), True)
        ...
        params = {
            ...
            'building_type_determination_values': json_dict.get('data', {}).get('building_type_determination', {}).get('residential_values', ['戸建', '共同住宅']),
        }
        ...
        for item in input_source:
            ...
            E014(
                ...
                int(progress_percent_job),
                progress_percent
            )
            main_csv = output_e014
            progress_percent_job = int(progress_percent_job) + progress_percent
...
def e011(join_option, params, output_directory, job_id, columns):
    ...
    if address_of_lot_number:
        ...
        columns.extend(['building_id_address_of_lot_number', '地番住所_address_of_lot_number', '緯度_address_of_lot_number', '経度_address_of_lot_number'])
