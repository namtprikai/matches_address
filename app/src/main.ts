...
async function createDummyDataSetResults(): Promise<void> {
  ...
  const d902 = Array.from({ length: 1 }, (_, i) => i + 1).map((i) => {
    const filePath = path.resolve(`./public/D902/min.json`);
  ...
  const referenceYears = [2019, 2020, 2021, 2022, 2023];

  // 年度ごとにダミーデータを生成する
  referenceYears.map(async (year) => {
