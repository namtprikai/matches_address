...
import { type data_set_detail_areas, data_set_detail_buildings } from "../schema";
...
export const filterDataSetForTable = ((_: unknown, { resultId, type, columns }: { resultId: number } & (
    ...
    { type: "area"; columns: (keyof typeof data_set_detail_areas.$inferSelect)[]; })): FilterDataSetForTableResponse => {
    ...
    if (type === "area") {
        ...
        return {
            // @ts-expect-error TODO: この辺りの型定義は別途修正が必要
            columns: columns.map(column => ({ key: column, label: DATA_SET_DETAIL_AREA_COLUMN_CONFIG[column].label, unit: DATA_SET_DETAIL_AREA_COLUMN_CONFIG[column].unit })),
            data: all.map((row) => {
                ...
                const formattedRow = rowArray.reduce((acc, [key, value]) => {
                    return {
                        ...acc,
                        // @ts-expect-error TODO: この辺りの型定義は別途修正が必要
                        [key]: formatChartValue(value, DATA_SET_DETAIL_AREA_COLUMN_CONFIG[key].percentage, 2)
                    }
                }, {})
