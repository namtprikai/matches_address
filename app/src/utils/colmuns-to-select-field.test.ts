...
describe("表形式でのView表示向けのデータフィルタリング機能のテスト", () => {
    it('集計単位が建物の場合に指定したKeyの配列から、select向けのオブジェクトに変換する', () => {
        const keys = ["normalized_address", "name"] satisfies (keyof typeof data_set_detail_buildings.$inferSelect)[];
    ...
    it('集計単位が地域の場合に指定したKeyの配列から、select向けのオブジェクトに変換する', () => {
        const keys = ["id", "created_at"] satisfies (keyof typeof data_set_detail_areas.$inferSelect)[];
