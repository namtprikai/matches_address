...
def assign_points_to_buildings(buildings_gdf, points_gdf, mul, crs, point_selected_column, option, building_id):
    ...
    if option == 1:
        ...
        combined_gdf['geometry_plateau'] = gpd.GeoSeries(
            combined_gdf['geometry_plateau'], 
            # crs=combined_gdf.crs
        )
        # .transform(_drop_z)
        # combined_gdf['geometry_plateau'] = transform_to_wgs84(
        #     gpd.GeoSeries(combined_gdf['geometry_plateau'], 
        #                   crs=combined_gdf.crs
        #                  ), crs
        # )
    ...
    combined_gdf = combined_gdf.drop_duplicates(['ID'],ignore_index=True)
...
def process_data(tatemono_path, water_supply_path, gpkg_path, ken, sikuchoson, option, output_type, input_zip_file=None, output_path=None, job_id=None, db_path=None, building_id='buildingID', input_source=[]):
    ...
    try:
        ...
        setai_col = [ col for col in tatemono_use_point_add_keycode.columns if "世帯コード" in col ][0]
        address_col = [ col for col in tatemono_use_point_add_keycode.columns if "正規化住所" in col ][0]
        tatemono_use_point_add_keycode = tatemono_use_point_add_keycode.drop_duplicates(
            [suido_col, setai_col, address_col, 'buildingID'], ignore_index=True
        )
