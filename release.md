...
- **空き家確率の推定**
  ...
  - 構築した AI モデルを用い、推定したい時点のデータから建物ごとに空き家確率を算出
  - 国勢調査の小地域単位や独自のポリゴン単位で集計・可視化構築した AI モデルを用い、推定したい時点のデータから建物ごとに空き家確率を算出

- **可視化ダッシュボードによる分析**
  - 地図表示による地域空き家の可視化
  - 棒グラフによる空き家確率の比較（町丁単位などにおける空き家件数の比較など）
  - 円グラフによる空き家確率の分布（立物総件数に対する空き家件数の割合を分析するなど）
  - 折れ線グラフによる空き家確率の推移（空き家件数の対象年度ごとの推移を分析するなど）
  - 表による空き家確率及び関連する属性の表示
  - フィルターによる条件選択
...
構築済みモデルと、推定したい時点の名寄せ済みデータを指定し、家屋ごとの空き家確率を算出。
